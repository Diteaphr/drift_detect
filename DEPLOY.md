...
這個 dashboard（`app.py`，含一般使用者 / 工程師兩個介面）只跑 repo 內既有的資料，
不接受使用者上傳。舊的 `monitor_app.py` 保留為轉接檔，直接指向 `app.py`。
部署步驟如下。
...
- 選單只列出每種 drift type 各 2 份的精選資料（見 `core/run.py` 的 `DEMO_DRIFT_DIRS` / `DEMO_PER_TYPE`）
- `data/long_drift/` 的 100 萬筆長版串流**沒有入庫**（57 MB，已 gitignore），
  所以雲端版的選單不會出現「長版 ×10」。要在本機看，先跑
...
3. **Main file path** 填 `app.py`（填舊的 `monitor_app.py` 也能跑）
4. 按 **Deploy**

Streamlit Cloud 會自動讀 `requirements.txt` 安裝依賴。約 2–3 分鐘後就有公開網址。

## 記憶體注意事項
免費方案約 1GB RAM，而資料檔是 100k 列。工程師介面的 `max_steps` 預設 20000
（一般使用者介面固定跑 20000 筆，見 `views/operator.py`），
